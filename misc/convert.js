...
let one = require('onecolor')

let sortFunction
let projectRoot = appRoot.path
...
function convert () {
  ...
  let files = fs.readdirSync(toConvertAbsoluteBasePath)
...
function processFiles (files, fidx) {
  if (fidx < files.length) {
    ...
    if (isSupportedExtension(extension)) {
      let filePath = path.join(toConvertAbsoluteBasePath, file)
...
function createPreviewImage (files, fidx, filePath, file, index) {
  // create letious preview images

  gm(filePath).resize(null, resolutions[index].height).autoOrient().quality(95).write(assetsAbsoluteBasePath + resolutions[index].name + '/' + file, function (err) {
    if (err) {
      throw err
    }
...
function provideImageInformation (imageMetadataArray, imgMetadataIdx, resolutions, resolutionIdx) {
  let imgMetadata = imageMetadataArray[imgMetadataIdx]
  let resolution = resolutions[resolutionIdx]

  let filePath = assetsAbsoluteBasePath + resolution.name + '/' + imgMetadata.name
  ...
  gm(filePath).size(function (err, size) {
    ...
    if (resolutions.length - 1 == resolutionIdx) {
      gm(filePath).resize(250, 250).colors(1).toBuffer('RGB', function (err, buffer) {
        if (err) {
          throw err
        }
...
function sortByPrimaryColor () {
  ...
  for (let i = 0; i < iterations; i++) {
    let specificColorSpectrum = imageMetadataArray.filter(function (imageMetadata) {
  ...
  let sortedColorsArray = flatten(sortedColorsArray)
